...
<div class="my-6">
    ...
    <div class="flex justify-between items-center mb-4 space-x-4">
        <form method="GET" action="{{ route('category.index') }}" class="flex items-center space-x-4 flex-grow">
        ...
        <a href="{{ route('category.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-lg shadow-md transition duration-200">Add New Category</a>
    </div>
        ...
        <table class="min-w-full bg-white border-collapse border border-gray-200 shadow-md rounded-lg overflow-hidden">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($categories as $category)
                    <tr id="category-{{ $category->id }}" class="hover:bg-gray-100 transition duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if ($category->image)
                                <img src="{{ asset('storage/' . $category->image) }}" alt="Category Image" class="h-12 w-12 object-cover rounded">
                            @else
                                No Image
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="{{ $category->status == '1' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }} inline-block px-2 py-1 rounded-lg">
                                {{ $category->status == '1' ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        
                        <td class="px-6 py-4 whitespace-nowrap">
                            <a href="{{ route('category.edit', $category->id) }}" class="text-blue-500 hover:text-blue-600">
                                <i class="fas fa-edit"></i>
                            </a>
                            <button onclick="showCategoryDetails({{ $category->id }})" class="text-gray-500 hover:text-gray-600 ml-2">
                                <i class="fas fa-eye"></i>
                            </button>
                            <button onclick="deleteCategory({{ $category->id }})" class="text-red-500 hover:text-red-600 ml-2">
...
<!-- Modal HTML -->
<div id="category-details-modal" class="fixed z-10 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen px-4 py-8">
        <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all max-w-lg w-full">
            ...
            <div class="px-4 py-5 sm:px-6">
                <div id="category-details-content" class="space-y-4">
        ...
        if (selected.length > 0) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`/admin/category/bulk-delete`, {
                        method: 'DELETE',
                        headers: {
